...
abstract class entityMatch {
    ...
    public abstract function find_matching_entities($entity_id,$entity_name,$dhisid,$dhiscode);
...
<csd:requestParams xmlns:csd='urn:ihe:iti:csd:2013'>
    ...
    public function is_marked_duplicate($id1,$id2 ) {
        ...
        if ($this->extract($ent2,"/csd:otherID[@assigningAuthorityName='https://dhis.moh.go.tz' and @code='id' and ./text()='{$id1}']",false)) {
            return true;
        } else {
            return false;
        }
...
class entityMatchPotentialDuplicates extends entityMatch {
    public function find_matching_entities($entity_id,$entity_name,$dhisid,$dhiscode) {
...
class entityMatchLevenshtein extends entityMatch {
    ...
    public function cache_dhis_facilities($limit_districts,$doc_name,$first_row,$max_rows) {
      ...
      foreach ($entities->facilityDirectory->children("urn:ihe:iti:csd:2013") as $facility) {
        ...
        foreach($facility->otherID as $otherID) {
          ...
          $code = (string)$otherID;
          else if($otherID->attributes()->code == "dhisid")
          $id = (string)$otherID;
        }

        $district = (string)$facility->parent->attributes()->entityID;
    ...
    public function cache_dhis_districts($limit_districts,$doc_name) {
      $results =array();
              $csr = "<csd:requestParams xmlns:csd='urn:ihe:iti:csd:2013'>
                      </csd:requestParams>";
    $urn = "urn:openhie.org:openinfoman-tz:select_dhis_district";
    $entity = $this->exec_request($doc_name,$csr,$urn);
    $districts = json_decode($entity,true);
    foreach ($districts["districts"] as $distr) {
      $mod[$distr["id"]] = $distr["name"];
    }
    return $mod;
    }
    ...
    public function cache_hfr_facilities($doc_name) {
      ...
      */
      foreach ($entities->facilityDirectory->children("urn:ihe:iti:csd:2013") as $facility) {
        ...
        $hfrcode = false;
        foreach($facility->otherID as $otherID) {
          if($otherID->attributes()->assigningAuthorityName == "https://dhis.moh.go.tz" and $otherID->attributes()->code == "id")
            $dhisid[] = (string)$otherID;
        ...
        if($dhisid)
        $facDetails[$doc_name][$uuid]["dhisid"] = $dhisid;
        if($hfrcode)
    ...
    public function find_matching_entities($src_entity_id,$src_entity_name,$dhisid,$dhiscode) {
      $strs=explode(" ",$src_entity_name);
       ...
       foreach ($this->target_entities[$this->target_doc_name] as $target_entity_id=>$target_entity) {
        ...
        if(array_key_exists("dhisid",$target_entity) and in_array($dhisid,$target_entity["dhisid"])) {
          //$rankings=array();
          $dup_found = true;
      		$lev=0;
      ...
      if(array_key_exists("hfrcode",$target_entity) and in_array($dhiscode,$target_entity["hfrcode"])) {
        $lev=0;
      }
        ...
        foreach($_POST['duplicate'][$source_id] as $target_id) {
          $csr = "<csd:requestParams xmlns:csd='urn:ihe:iti:csd:2013'>
                    <csd:id entityID='$target_id'>
                      <csd:otherID code='id' assigningAuthorityName='https://dhis.moh.go.tz'>$source_id</csd:otherID>
                    </csd:id>
                </csd:requestParams>";
...
echo "<a href='index.php'>Return</a>";
  ...
  if($options) {
    echo "<select name='target[$dhisid]' size='5' name='target[$dhisid]'>";
    echo $options;
    echo "</select>";
  }
  else
...
$entity_match->dhis_facilities = $entity_match->cache_dhis_facilities($districts,$src_doc_name,$first_row,$max_rows);
$entity_match->dhis_districts = $entity_match->cache_dhis_districts($districts,$src_doc_name);
...
foreach ($entity_match->dhis_facilities[$src_doc_name] as $entity_id=>$entity) {
  $dhisid = $entity["id"];
  $dhiscode = $entity["code"];
  $dhisdistrict = $entity_match->dhis_districts[$entity["district"]];
  $dhisname = $entity["name"];
	$rankings = $entity_match->find_matching_entities($entity_id,$dhisname,$dhisid,$dhiscode);
  if(array_key_exists("dup",$rankings) and count($rankings["dup"])>1)
  echo "<tr bgcolor='	#EC7063'>";
  else
  echo "<tr>";
	echo "<td>$count</td><td><input type='hidden' name=source[$dhisid] value=".$dhisid.">".$dhisname;
  echo "<br>&nbsp;&nbsp;&nbsp;&nbsp;Code: $dhiscode";
  echo "<br>&nbsp;&nbsp;&nbsp;&nbsp;ID: $dhisid";
  echo "<br>&nbsp;&nbsp;&nbsp;&nbsp;District: $dhisdistrict";
  echo "</td>";
	$count++;
      ...
      if(count($values["dhisid"])>1)
      echo "<b><font color='red'>";
      echo "<input type='hidden' value='$uuid' name='duplicate[$dhisid][$uuid]'>";
      if(count($values["dhisid"])>1)
      echo "<blink>";
			echo $values["name"];
      if(count($values["dhisid"])>1)
      ...
      if(count($values["dhisid"])>1) {
        echo "Also Mapped to ";
        $displayed = 0;
        foreach($values["dhisid"] as $dhisdupid) {
          if($dhisid != $dhisdupid){
            $displayed++;
            if($displayed>1)
            echo ",";
   	...
   	foreach($rankings as $lev=>$rankings_pull) {
   		...
   		echo "<font color='$colors[$match_index]'><label id=label$lev$dhisid onclick='show(`$lev$dhisid`,`$level_message`)'><b>+</b>Show The $level_message</label><br><div id='$lev$dhisid' style='display:none'>";
   		foreach($rankings_pull as $id=>$close) {
   			echo "<input type='radio' onclick='clear_search($dhisid)' name='target[$dhisid]' value='$id'>".$close["name"];
    ...
    echo "<table><tr><td>Search </td><td><input type='text' onkeyup='search_target(this.value,$dhisid)'></td></tr>";
    echo "<tr><td></td><td id='search$dhisid'></td></tr></table>";
   	echo "<input type='radio' name=target[$dhisid] value='no_duplicate'>No Match<br>";
   	echo "</td></tr>";
}
