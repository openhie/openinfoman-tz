...
declare function page:get_actions($search_name) {
  ...
  return 
    ...
    return string($act)
    ,for $act in $ufunction//csd:extension[  @urn='urn:openhie.org:openinfoman:adapter:resourcemap:action']/@type
    return string($act)
  )
};
...
declare
  %rest:path("/CSD/csr/{$doc_name}/careServicesRequest/{$search_name}/adapter/resourcemap")
  %output:method("xhtml")
  function page:show_endpoints($search_name,$doc_name) 
{  
    if (not(page:is_resourcemap($search_name)) ) 
      then ('Not a ResourceMap Compatible stored function'    )
    else 
      let $actions := page:get_actions($search_name)
      let $contents := 
      <div>
        <h2>ResourceMap Operations on {$doc_name}</h2>
        { 
          if ($actions = 'upload')  
	  then
	   <span>
...
declare updating
  %rest:path("/CSD/csr/{$doc_name}/careServicesRequest/{$search_name}/adapter/resourcemap/upload")
  %rest:POST
